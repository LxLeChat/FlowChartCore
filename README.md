...
There is a parameter ``-codeastext`` that will render discovered  code for a given ``codenode`` and display it instead of just a ``codeblock``. We will this the difference in the examples.
```
PS > $x=Get-FLowChartNode -ScriptBlock $sb
